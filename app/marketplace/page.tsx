...
export default function MarketplacePage() {
  ...
  const handleBuy=async(asset:Asset)=>{
    ...
    try{
      ...
      const tx=await signAndSubmitTransaction({data:{function:"0x1::coin::transfer",typeArguments:["0x1::aptos_coin::AptosCoin"],functionArguments:[asset.owner,octas.toString()]}});
      await fetch("/api/buy",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:asset.id,owner:account.address.toString(),txHash:tx.hash})});
      setSuccess(asset.id); setTimeout(()=>setSuccess(null),3000);
      await fetchAssets();
            ...
            <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:"24px"}}>
              <div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"10px",marginTop:"9px",flexWrap:"wrap"}}>
                  <span style={{fontSize:"13px",color:"rgba(255,255,255,0.28)",fontFamily:"'Outfit',sans-serif"}}>Buy & sell using <span style={{color:"rgba(180,150,255,0.75)",fontWeight:600}}>$APT</span> on Aptos Testnet</span>
                  {aptPrice&&(
              ...
              <div style={{display:"flex",gap:"10px",flexWrap:"wrap"}}>
                {[
                  ...
                  {label:"Volume",       value:totalVol.toFixed(2)+" $APT"},
                ...
                return (
                  <div key={asset.id} className="mp-card" style={{animationDelay:`${idx*0.04}s`}} onClick={()=>setDetailAsset(asset)}>
                    ...
                    <div style={{position:"relative",aspectRatio:"1",background:"rgba(255,255,255,0.02)",overflow:"hidden"}}>
                      ...
                      {/* Price badge */}
                      {!soldOut&&<div style={{position:"absolute",bottom:"10px",right:"10px",padding:"4px 10px",borderRadius:"8px",background:"rgba(6,5,15,0.88)",backdropFilter:"blur(8px)",border:"1px solid rgba(108,56,255,0.3)",fontSize:"11px",fontWeight:700,color:"rgba(180,150,255,0.9)",fontFamily:"'Outfit',sans-serif"}}>{asset.price} {currency}</div>}
                    </div>
                    {/* Body */}
                    <div style={{padding:"14px 16px 16px",display:"flex",flexDirection:"column",gap:"11px"}}>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div>
                          ...
                          <p style={{fontSize:"15px",fontWeight:800,color:soldOut?"rgba(255,255,255,0.22)":"rgba(160,130,255,0.95)",fontFamily:"'Outfit',sans-serif",letterSpacing:"-0.01em"}}>{soldOut?"—":`${asset.price} ${currency}`}</p>
                          {aptPrice&&!soldOut&&<p style={{fontSize:"10px",color:"rgba(255,255,255,0.2)",fontFamily:"'Outfit',sans-serif"}}>≈ ${(asset.price*aptPrice).toFixed(2)}</p>}
                        </div>
                      ...
                      {owner
                        ...
                        : soldOut
                          ...
                          : <button className="mp-buy" onClick={e=>{e.stopPropagation();handleBuy(asset);}} disabled={!!buying}>
                              {buying===asset.id?<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"7px"}}><span className="spin" style={{width:"13px",height:"13px",border:"2px solid rgba(255,255,255,0.35)",borderTopColor:"white",borderRadius:"50%",display:"inline-block"}}/>Confirming...</span>:`Buy · ${asset.price} ${currency}`}
                            </button>}
                    </div>
                  </div>
                );
            ...
            <div className="mp-modal">
              ...
              <div style={{padding:"28px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px",marginBottom:"22px"}}>
                  {[
                    {label:"Price",     value:`${detailAsset.price} ${currency}`, sub:aptPrice?`≈ $${(detailAsset.price*aptPrice).toFixed(2)}`:"", color:"rgba(160,130,255,0.95)"},
                ...
                {detailAsset.listTxHash&&(
                  <a href={`https://explorer.aptoslabs.com/txn/${detailAsset.listTxHash}?network=testnet`} target="_blank" rel="noopener noreferrer"
                ...
                {!isOwner(detailAsset)&&!isSoldOut(detailAsset)&&(
                  <button className="mp-buy" onClick={()=>{setDetailAsset(null);handleBuy(detailAsset);}} disabled={!!buying}>
                    {buying===detailAsset.id?<span style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}><span className="spin" style={{width:"14px",height:"14px",border:"2px solid rgba(255,255,255,0.35)",borderTopColor:"white",borderRadius:"50%",display:"inline-block"}}/>Confirming...</span>:`Buy Now · ${detailAsset.price} ${currency}`}
                  </button>
                )}
                {isOwner(detailAsset)&&(
